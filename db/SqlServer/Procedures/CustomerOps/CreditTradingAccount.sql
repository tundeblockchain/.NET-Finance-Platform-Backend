/*
  Procedure : dbo.CreditTradingAccount
  Purpose   : Credits Settled on a TradingAccount. Idempotent via BrokerMutation.IdempotencyKey.
  Dated     : 2026-07-12
*/
SET ANSI_NULLS ON;
SET QUOTED_IDENTIFIER ON;
GO
CREATE OR ALTER PROCEDURE dbo.CreditTradingAccount
...
BEGIN
    ...
    IF @Amount <= 0
    BEGIN
        THROW 50024, 'Credit amount must be positive.', 1;
    END
    ...
    IF NOT EXISTS (SELECT 1 FROM dbo.TradingAccount WHERE Id = @AccountId)
    BEGIN
        ROLLBACK TRAN;
        THROW 50025, 'Trading account was not found.', 1;
    END
