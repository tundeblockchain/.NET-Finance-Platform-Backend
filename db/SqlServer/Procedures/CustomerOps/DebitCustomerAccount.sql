/*
  Procedure : dbo.DebitCustomerAccount
  Purpose   : Debits Settled on a CustomerAccount when available funds suffice. Idempotent via BrokerMutation.IdempotencyKey.
  Dated     : 2026-07-12
*/
SET ANSI_NULLS ON;
SET QUOTED_IDENTIFIER ON;
GO
CREATE OR ALTER PROCEDURE dbo.DebitCustomerAccount
...
BEGIN
    ...
    IF @Amount <= 0
    BEGIN
        THROW 50022, 'Debit amount must be positive.', 1;
    END
    ...
    IF NOT EXISTS (
        SELECT 1 FROM dbo.CustomerAccount
        WHERE Id = @AccountId AND (Settled - Reserved) >= @Amount
    )
    BEGIN
        ROLLBACK TRAN;
        THROW 50023, 'Insufficient funds in customer account.', 1;
    END
